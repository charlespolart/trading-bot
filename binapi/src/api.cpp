...
api::new_order(
    ...
    // TODO
    const char *response_type = 1 /*type == e_type::market*/
        ? "FULL"
        : "RESULT"
    ;
...
api::result<new_order_resp_type> api::new_test_order(
    ...
    static const char *response_type = "RESULT";
...
api::result<new_orderOCO_resp_type> api::new_orderOCO(
     const char *symbol
    ,const e_side side
    ,const char *amount
    ,const char *price
    ,const char *stopPrice
    ,const char *stopLimitPrice
    ,const e_time stopLimitTimeInForce
    ,new_orderOCO_cb cb
) {
    const char *side_str = order_side_to_string(side);
    assert(side_str);

    const char *stopLimitTimeInForce_str = order_time_to_string(stopLimitTimeInForce);
    assert(stopLimitTimeInForce_str);
    ...
    const impl::init_list_type map = {
        ...
        ,{"stopLimitTimeInForce", stopLimitTimeInForce_str}
    };

    return pimpl->post(true, "/api/v3/order/oco", boost::beast::http::verb::post, map, std::move(cb));
}
