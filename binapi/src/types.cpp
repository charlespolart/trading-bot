...
new_orderOCO_resp_type new_orderOCO_resp_type::parse(const char *str, std::size_t len) {
    (void)str;
    (void)len;
    new_orderOCO_resp_type res{};

    return res;
}

std::ostream &operator<<(std::ostream &os, const new_orderOCO_resp_type &o) {
    (void)o;
    os << "{\"ok\":" << "ok" << "}";
