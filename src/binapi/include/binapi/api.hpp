...
struct api {
    ...
    new_order(
        ...
        ,const std::string &quoteOrderQty
        ...
        return new_order(
            ...
            ,(quoteOrderQty.empty() ? nullptr : quoteOrderQty.c_str())
    ...
    // NOTE: if 'ec' not zero - the 'res' arg is in undefined state.
    using new_orderOCO_cb = std::function<bool(const char *fl, int ec, std::string errmsg, new_orderOCO_resp_type res)>;
    result<new_orderOCO_resp_type>
    new_orderOCO(
         const std::string &symbol
        ,const e_side side
        ,const std::string &amount
        ...
        ,const std::string &stopLimitPrice
